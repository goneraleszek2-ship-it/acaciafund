...
const CONFIG = {
  // Target URLs to test
  urls: [
    'https://www.acaciafund.org/',
    'https://www.acaciafund.org/learn/',
    'https://www.acaciafund.org/blog/'
  ],
  ...
  // Delay between each URL test (to avoid rate limiting)
  delayBetweenTests: 60 * 1000, // 60 seconds
  
  // Retry configuration
  maxRetries: 2,
  baseDelay: 30000, // 30 seconds base delay
  maxDelay: 120000  // 120 seconds max delay
};
...
async function runPerformanceAudit() {
  ...
  console.log(`Testing ${CONFIG.urls.length} URLs on ${CONFIG.strategy}`);
  if (CONFIG.apiKey) {
  ...
  for (const url of CONFIG.urls) {
    try {
      ...
      const report = generateReport(psiData, url);
      ...
      if (!CONFIG.apiKey) {
        console.log('Waiting 30 seconds before next request (free tier limit)...');
