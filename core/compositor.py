...
def render_timeline(
    events: list[dict], pillar: str = "", width: int = 600, height: int | None = None
) -> str:
...
def render_flow(
    steps: list[dict], pillar: str = "", width: int = 600, height: int | None = None
) -> str:
...
def render_comparisons(
    comparisons: list[dict], pillar: str = "", width: int = 600, height: int | None = None
) -> str:
    ...
    for i, hdr in enumerate(headers):
        ...
        parts.append(
            _text(
                ...
                y_start + header_h - 6,
                hdr,
                cls="var(--color-text-secondary, #475569)",
                size=9,
                fw="600",
            )
        )
    ...
    for i, comp in enumerate(comparisons):
        ...
        display = (
            f"{entity} ({metric})" if metric and metric not in ("comparison", "change") else entity
        )
        parts.append(
            _text(x_start + 6, ry + row_h - 8, display, cls="var(--color-text, #e8e6e3)", size=10)
        )
        parts.append(
            _text(x_start + col_w[0] + 6, ry + row_h - 8, va, cls=pal["text"], size=10, fw="600")
        )
        if vb:
            parts.append(
                _text(
                    x_start + col_w[0] + col_w[1] + 6,
                    ry + row_h - 8,
                    vb,
                    cls="var(--color-text, #e8e6e3)",
                    size=10,
                )
            )
...
def auto_compose(body_text: str, pillar: str = "", width: int = 600) -> list[dict[str, Any]]:
    ...
    """
    from core.extractors import extract_comparisons, extract_flow, extract_timeline
