...
_SI_CACHE: dict[str, str] = {}


def resolve_topic_icon(key: str) -> str | None:
    """Resolve a TOPIC_ICONS entry to its SVG path string(s).

    Abstract icons return the stored path data directly.
    Brand icons are resolved from the simpleicons package and cached.
    """
    entry = TOPIC_ICONS.get(key)
    if not entry:
        return None
...
def render_topic_icon(key: str, accent: str = "currentColor") -> str:
    """Render a TOPIC_ICONS entry as an inline SVG <g> element suitable for templates.

    Abstract icons get fill="none" stroke="currentColor".
    Brand icons get fill="currentColor".
    Returns empty string if the icon cannot be resolved.
    """
    path_str = resolve_topic_icon(key)
    if not path_str:
        return ""

    entry = TOPIC_ICONS.get(key)
    is_brand = entry and entry.get("type") == "brand"

    if is_brand:
        return f'<g color="{accent}" fill="currentColor">{path_str}</g>'
    else:
        return (
            f'<g color="{accent}" fill="none" stroke="currentColor" '
            f'stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5">'
            f"{path_str}</g>"
        )


def get_brand_topic_slugs() -> list[str]:
    """Return all Simple Icons slugs used in TOPIC_ICONS brand entries."""
    return [e["slug"] for e in TOPIC_ICONS.values() if e["type"] == "brand"]
...
TOPIC_ICONS: dict[str, dict] = {
    # === ABSTRACT ICONS (hand-drawn, stroke-based, fill="none") ===
    # AML pillar
    "regulation": {
        "type": "abstract",
        "paths": '<path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/>',
    },
    "compliance": {
        "type": "abstract",
        "paths": '<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 12l2 2 4-4"/>',
    },
    "crypto": {
        "type": "abstract",
        "paths": '<circle cx="12" cy="12" r="10"/><path d="M12 6v12M9 9h6l-3 6-3-6z"/>',
    },
    "fraud": {
        "type": "abstract",
    ...
    "semiconductor": {
        "type": "abstract",
        "paths": '<path d="M6 6h12v12H6z"/><path d="M8 8h8v8H8z"/><path d="M10 10h4v4h-4z"/>',
    },
    "ai": {
        "type": "abstract",
        "paths": '<circle cx="12" cy="12" r="10"/><path d="M12 8v8M8 12h8"/><circle cx="12" cy="12" r="3"/>',
    },
    "stock_market": {"type": "abstract", "paths": '<path d="M2 20h20M6 16l4-4 4 4 4-8"/>'},
    "startup": {
        "type": "abstract",
        "paths": '<path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M12 22V12"/>',
    },
    "manufacturing": {
        "type": "abstract",
    ...
    "dna": {
        "type": "abstract",
        "paths": '<path d="M8 2c0 0 0 20 8 20M16 2c0 0 0 20-8 20M8 12h8"/>',
    },
    "quantum": {
        "type": "abstract",
        "paths": '<circle cx="12" cy="12" r="3"/><circle cx="16" cy="8" r="1.5"/><circle cx="16" cy="16" r="1.5"/><circle cx="8" cy="8" r="1.5"/><circle cx="8" cy="16" r="1.5"/><path d="M12 3v2M12 19v2M3 12h2M19 12h2"/>',
    },
    "brain": {
        "type": "abstract",
        "paths": '<path d="M12 2C8 2 4 5 4 10c0 3 2 5 2 5s0 5 6 5 6-5 6-5 2-2 2-5c0-5-4-8-8-8z"/><path d="M9 10h6M10.5 12h3M10 8h4"/>',
    },
    "space": {
        "type": "abstract",
        "paths": '<circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a16 16 0 010 20 16 16 0 010-20z"/><ellipse cx="12" cy="12" rx="4" ry="10"/>',
    },
    "climate": {
        "type": "abstract",
        "paths": '<circle cx="12" cy="12" r="10"/><path d="M12 2a8 8 0 000 16"/><path d="M8 6l2 2M16 6l-2 2"/>',
    },
    # Data Engineering subtopics (abstract)
    "pipeline": {
        "type": "abstract",
        "paths": '<path d="M3 12h5l2-3 4 6 2-3h5"/><circle cx="3" cy="12" r="2"/><circle cx="21" cy="12" r="2"/><path d="M10 6h4M10 18h4"/>',
    },
    "storage": {
        "type": "abstract",
        "paths": '<ellipse cx="12" cy="6" rx="8" ry="3"/><path d="M4 6v6c0 1.66 3.58 3 8 3s8-1.34 8-3V6"/><path d="M4 12v6c0 1.66 3.58 3 8 3s8-1.34 8-3v-6"/>',
    },
    "quality": {
        "type": "abstract",
        "paths": '<path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>',
    },
    "streaming": {
        "type": "abstract",
        "paths": '<path d="M18.36 6.64a9 9 0 11-12.73 0M12 2v8M9 7l3-3 3 3"/>',
    },
    "infrastructure": {
        "type": "abstract",
        "paths": '<rect x="4" y="4" width="16" height="16" rx="2"/><path d="M9 4v16M15 4v16M4 9h16M4 15h16"/>',
    },
}
# === BRAND ICONS (Simple Icons — filled, resolved from simpleicons package) ===
# Data Engineering
_BRANDS_DATA: dict[str, set[str]] = {
    "apachekafka": {"kafka", "confluent", "debezium", "avro", "ksqldb", "schema registry"},
    "apacheairflow": {"airflow", "apache airflow", "dag", "orchestrator"},
    "apacheflink": {"flink", "apache flink", "stream processor"},
    "apachespark": {"spark", "apache spark", "pyspark", "databricks", "spark sql"},
    "dbt": {"dbt", "data build tool", "dbt mesh", "data transformation"},
    "delta": {"delta lake", "delta", "delta table"},
    "trino": {"trino", "presto", "query engine"},
    "duckdb": {"duckdb", "duck db", "analytics"},
    "mlflow": {"mlflow", "model registry", "mlflow tracking"},
    "dvc": {"dvc", "data version control", "pipeline versioning"},
    ...
    "github": {"github", "gh", "github actions"},
    ...
    "microsoftazure": {"azure", "microsoft azure", "synapse", "fabric", "azure data lake"},
    "googlecloud": {"gcp", "google cloud", "bigquery", "dataflow", "pubsub"},
    "cloudflare": {"cloudflare", "cf", "cdn"},
    "nginx": {"nginx", "nginx", "reverse proxy"},
    "linux": {"linux", "ubuntu", "debian"},
    "ubuntu": {"ubuntu", "linux"},
    "json": {"json", "json schema"},
    "opentelemetry": {"opentelemetry", "otel", "observability"},
    ...
    # Markets / Tech giants
    "nvidia": {"nvidia", "gpu", "cuda", "h100", "a100"},
    ...
    "microstrategy": {"microstrategy", "mstr", "bitcoin treasury"},
    "fedora": {"fedora linux", "fedora os"},
    # Finance / Payments / Banking
    "stripe": {"stripe", "payment", "payments", "payment gateway", "checkout"},
    "visa": {"visa", "visa card", "card payment", "debit card"},
    "mastercard": {"mastercard", "maestro"},
    "discover": {"discover", "discover card", "discover financial"},
    "square": {"square", "square payment", "pos", "point of sale"},
    "paypal": {"paypal", "online payment", "paypal transaction"},
    "venmo": {"venmo", "peer to peer payment", "p2p payment"},
    "wise": {
        "wise",
        "transferwise",
    ...
    "revolut": {"revolut", "digital banking", "neobank", "challenger bank"},
    "monzo": {"monzo", "monzo bank"},
    "starlingbank": {"starling bank", "starling"},
    "nubank": {"nubank", "nu holdings"},
    "alipay": {"alipay", "mobile payment", "digital wallet"},
    "applepay": {"apple pay", "contactless payment", "nfc payment", "tap to pay"},
    "googlepay": {"google pay", "tap pay", "mobile wallet"},
    "samsungpay": {"samsung pay", "mobile wallet"},
    "amazonpay": {"amazon pay", "checkout payment"},
    "payoneer": {"payoneer", "freelance payment"},
    "contactlesspayment": {"contactless payment", "tap pay", "nfc payment"},
    "quickbooks": {"quickbooks", "accounting", "bookkeeping"},
    "xero": {"xero", "accounting", "invoicing"},
    "bankofamerica": {"bank of america", "bofa"},
    "paytm": {"paytm", "mobile wallet india"},
    "razorpay": {"razorpay", "payment gateway india"},
    # Crypto / Blockchain
    "bitcoin": {"bitcoin", "btc", "cryptocurrency", "blockchain", "digital gold"},
    "ethereum": {"ethereum", "eth", "defi", "smart contract", "evm"},
    "monero": {"monero", "xmr", "privacy coin", "anon", "untraceable"},
    "litecoin": {"litecoin", "ltc"},
    "ripple": {"ripple", "xrp", "cross border payment", "ripple net"},
    "xrp": {"xrp", "ripple net", "cross border settlement"},
    "stellar": {"stellar", "xlm", "lumens"},
    "polkadot": {"polkadot", "dot", "polkadot network"},
    "chainlink": {"chainlink", "link", "oracle", "chainlink oracle"},
    ...
    "coinbase": {"coinbase", "crypto exchange", "coinbase wallet"},
    "hive_blockchain": {"hive", "hive blockchain"},
    "hyperledger": {"hyperledger", "blockchain framework", "distributed ledger", "dlt"},
    "blockchaindotcom": {"blockchain.com", "blockchaindotcom"},
    "coinmarketcap": {"coinmarketcap", "crypto prices", "token price"},
    "signal": {"signal app", "signal messaging"},
    # Science / Space
    "nasa": {"nasa", "space", "jwst", "exoplanet", "astronomy"},
    # Tools
    "visualstudiocode": {"vscode", "visual studio code", "vs code"},
    "pycharm": {"pycharm"},
    "diagramsdotnet": {"diagrams.net", "draw.io", "diagram"},
}
for _slug, _kws in _BRANDS_DATA.items():
    TOPIC_ICONS[_slug] = {"type": "brand", "slug": _slug}
...
# Brand icon keyword mappings — built from TOPIC_ICONS brand entries.
# Checked first so specific tech names map directly to brand SVGs (Simple Icons)
# before falling back to general subtopic category matching.
BRAND_ICON_KEYWORDS: dict[str, set[str]] = {k: v for k, v in _BRANDS_DATA.items()}
...
def _fractal_fern(elems: list, seed: int, pal: dict, w: int, h: int, seq: list, count: int = 300):
    ...
    for i in range(count):
        ...
        r_size = 0.8 + t * 1.5
        elems.append(
            f'<circle cx="{px:.1f}" cy="{py:.1f}" r="{r_size:.1f}" fill="{c}" opacity="{op:.2f}"/>'
        )
...
def _fractal_hilbert(
    ...
    if depth <= 0:
        ...
        op = 0.15 + t * 0.35
        elems.append(
            f'<circle cx="{nx:.1f}" cy="{ny:.1f}" r="{sw:.1f}" fill="{c}" opacity="{op:.2f}"/>'
        )
        return
...
def _mirror_elements(elems: list, mirror_x: bool, mirror_y: bool, w: int, h: int) -> list:
    ...
    for el in elems:
        m = el
        if mirror_x:
            m = m.replace('x1="', f'x1="{-1 if "x1" in m else ""}')
...
def _generate_mist(seed: int, pal: dict, w: int, h: int, count: int, seq: list) -> list:
...
def _overlay_panel_svg(
    featured_image_url: str,
    fallback_icons: list | None,
    layer: str,
    pal: dict,
    width: int,
    height: int,
    ...
    if featured_image_url:
        ...
        if layer == "research":
            ...
            elems.append(
                f'<rect x="{ix + iw - 24}" y="{iy}" width="24" height="{ih}" fill="url(#{sid})"/>'
            )
        elif layer == "learn":
            sid = f"shadow-{layer}"
            defs.append(
            ...
            elems.append(f'<rect x="{ix}" y="{iy}" width="24" height="{ih}" fill="url(#{sid})"/>')
        elif layer == "knowledge":
            sid = f"shadow-{layer}"
            defs.append(
            ...
            elems.append(
                f'<rect x="{ix}" y="{iy + ih - 24}" width="{iw}" height="24" fill="url(#{sid})"/>'
            )
    elif fallback_icons:
        elems.append(
            f'<rect x="{ix}" y="{iy}" width="{iw}" height="{ih}"'
...
def generate_thumbnail_svg(
    ...
    icon_path = resolve_topic_icon(sub) or resolve_topic_icon("regulation") or ""
    ...
    if bg_v == 0:
        bg = (
            f'<linearGradient id="bg-{h[:8]}" x1="0" y1="0" x2="1" y2="1">'
        ...
        bg = (
            f'<radialGradient id="bg-{h[:8]}" cx="{30 + _det_rand_int(seed, seq[0], 0, 40)}%" '
        ...
        bg = (
            f'<linearGradient id="bg-{h[:8]}" x1="0" y1="1" x2="1" y2="0">'
            f'<stop offset="0" stop-color="{pal["bg"]}"/>'
    ...
    mist1 = _generate_mist(seed, pal, width, height, 8 + _det_rand_int(seed, seq[0], 0, 4), seq)
    seq[0] += 1
    lines.extend(mist1)

    # Fractal elements
    fractal_elems: list[str] = []
    ...
    if ftype == 0:
        ...
        for i in range(3):
            ...
            _fractal_koch(
                fractal_elems, seed + i, pal, width, height, x1, y1, x2, y2, depth, seq, i * 30
            )
        if mirror_x:
            seq[0] += 10
            _fractal_koch(
        ...
        seq[0] += 4
        _fractal_dragon(
            fractal_elems, seed, pal, width, height, start_x, start_y, end_x, end_y, depth, seq
        )
        ...
        seq[0] += 1
        _fractal_spiral(fractal_elems, seed, pal, width, height, width / 2, height / 2, seq, turns)
        ...
        oy = (height - size) / 2
        _fractal_hilbert(
            fractal_elems, seed, pal, width, height, ox, oy, size, 0, 0, size, depth, seq
        )

    lines.extend(fractal_elems)

    # Mist layer 2 (foreground, over fractal)
    mist2 = _generate_mist(
        seed + 1000, pal, width, height, 4 + _det_rand_int(seed, seq[0], 0, 4), seq
    )
    seq[0] += 1
    lines.extend(mist2)
...
def generate_og_image(
    ...
    if bg_v == 0:
        ...
        bg = (
            f'<radialGradient id="ogbg" cx="{40 + _det_rand_int(seed, seq[0], 0, 30)}%"'
    ...
    parts = (
        ...
        + [
            "<!-- Icon -->",
            f'<g transform="translate(50, 50) scale(1.8)" color="{pal["accent"]}">',
            f"  {icon_path}",
            "</g>",
        ]
    )
...
def generate_all_thumbnails(
    pillar_stories: dict[str, list[dict]], pillar_signals: dict[str, dict]
) -> dict[str, str]:
...
def source_bar_svg(breakdown: dict, width: int = 280, height: int = 80) -> str:
    ...
    for key in ("hn", "arxiv", "pubmed"):
        ...
        parts.append(
            f'<rect x="{x}" y="{y}" width="{seg_w}" height="{bar_h}" rx="3" fill="{c}" opacity="0.9"/>'
        )
        parts.append(
            f'<text x="{x + 6}" y="{y + 14}" fill="#fff" font-family="system-ui,sans-serif" font-size="10" font-weight="600">{label} {val}</text>'
        )
        x += seg_w
    # Scale ticks
    for pct in (0, 25, 50, 75, 100):
        ...
        parts.append(
            f'<line x1="{tx}" y1="{y + bar_h + 2}" x2="{tx}" y2="{y + bar_h + 6}" stroke="var(--color-text-muted, #94a3b8)" stroke-width="0.5"/>'
        )
        parts.append(
            f'<text x="{tx}" y="{y + bar_h + 16}" text-anchor="middle" fill="var(--color-text-secondary, #475569)" font-family="system-ui,sans-serif" font-size="7">{pct}%</text>'
        )
    parts.append("</svg>")
    return "\n".join(parts)
...
def sparkline_svg(
    values: list[float], color: str = "#22c55e", width: int = 160, height: int = 40
) -> str:
...
def bloom_chart_svg(questions: list, width: int = 280, height: int = 180) -> str:
    ...
    for i, level in enumerate(BLOOM_LABELS):
        ...
        parts.append(
            f'<text x="{label_w - 4}" y="{y + bar_h - 4}" text-anchor="end" fill="var(--color-text-secondary, #475569)" font-family="system-ui,sans-serif" font-size="9" font-weight="500">{level}</text>'
        )
        parts.append(
            f'<rect x="{label_w}" y="{y}" width="{bar_w}" height="{bar_h}" rx="3" fill="{c}" opacity="0.85"/>'
        )
        if bar_w > 20:
            parts.append(
                f'<text x="{label_w + 6}" y="{y + bar_h - 4}" fill="#fff" font-family="system-ui,sans-serif" font-size="9" font-weight="600">{cnt}</text>'
            )
        else:
            parts.append(
                f'<text x="{label_w + bar_w + 4}" y="{y + bar_h - 4}" fill="{c}" font-family="system-ui,sans-serif" font-size="9" font-weight="600">{cnt}</text>'
            )
    parts.append("</svg>")
    return "\n".join(parts)
...
def radar_svg(metrics: dict, width: int = 180, height: int = 180) -> str:
    ...
    for ring in (0.25, 0.5, 0.75, 1.0):
        ...
        parts.append(
            f'<polygon points="{" ".join(pts)}" fill="none" stroke="#2d2d4a" stroke-width="0.5" opacity="0.3"/>'
        )
    # Axis lines
    for a, lbl in zip(angles, labels):
        ...
        parts.append(
            f'<line x1="{cx}" y1="{cy}" x2="{x2:.1f}" y2="{y2:.1f}" stroke="#2d2d4a" stroke-width="0.5" opacity="0.4"/>'
        )
        parts.append(
            f'<text x="{lx:.1f}" y="{ly:.1f}" text-anchor="middle" dominant-baseline="middle" fill="var(--color-text-secondary, #475569)" font-family="system-ui,sans-serif" font-size="8">{lbl}</text>'
        )
    # Data triangle
    pts = []
    for a, v in zip(angles, vals):
        ...
        pts.append(f"{x:.1f},{y:.1f}")
    parts.append(
        f'<polygon points="{" ".join(pts)}" fill="#a855f7" opacity="0.2" stroke="#a855f7" stroke-width="1.5" stroke-linejoin="round"/>'
    )
    for p in pts:
        parts.append(
            f'<circle cx="{p.split(",")[0]}" cy="{p.split(",")[1]}" r="3" fill="#a855f7"/>'
        )
    # Center value labels
    for a, v in zip(angles, vals):
        r2 = radius * v * 0.6
        ...
        parts.append(
            f'<text x="{lx:.1f}" y="{ly:.1f}" text-anchor="middle" dominant-baseline="middle" fill="#c084fc" font-family="system-ui,sans-serif" font-size="8" font-weight="600">{v:.2f}</text>'
        )
    parts.append("</svg>")
    return "\n".join(parts)
...
def heatmap_svg(
    ...
    for i, row in enumerate(data):
        for j, val in enumerate(row):
            ...
            parts.append(
                f'<rect x="{x}" y="{y}" width="{cell_size}" height="{cell_size}" rx="3" fill="{fill}" opacity="0.85"/>'
            )
            parts.append(
                f'<text x="{x + cell_size // 2}" y="{y + cell_size // 2 + 1}" text-anchor="middle" dominant-baseline="middle" fill="#fff" font-family="system-ui,sans-serif" font-size="8" font-weight="600">{val:.1f}</text>'
            )
    if row_labels:
        for i, lbl in enumerate(row_labels):
            y = header_h + i * (cell_size + gap) + cell_size // 2 + 1
            parts.append(
                f'<text x="{label_w - 4}" y="{y}" text-anchor="end" dominant-baseline="middle" fill="var(--color-text-secondary, #475569)" font-family="system-ui,sans-serif" font-size="8">{lbl}</text>'
            )
    if col_labels:
        for j, lbl in enumerate(col_labels):
            x = label_w + j * (cell_size + gap) + cell_size // 2
            parts.append(
                f'<text x="{x}" y="{header_h - 4}" text-anchor="middle" fill="var(--color-text-secondary, #475569)" font-family="system-ui,sans-serif" font-size="8">{lbl}</text>'
            )
    parts.append("</svg>")
    return "\n".join(parts)
...
def donut_svg(breakdown: dict, width: int = 140, height: int = 140) -> str:
    ...
    parts.append(
        f'<text x="{cx}" y="{cy - 2}" text-anchor="middle" dominant-baseline="middle" fill="var(--color-text, #e8e6e3)" font-family="system-ui,sans-serif" font-size="14" font-weight="700">{total}</text>'
    )
    parts.append(
        f'<text x="{cx}" y="{cy + 12}" text-anchor="middle" dominant-baseline="middle" fill="var(--color-text-secondary, #475569)" font-family="system-ui,sans-serif" font-size="7">sources</text>'
    )
    # Legend below
    ly = height - 12
    lx_start = 10
    ...
    for key in ("hn", "arxiv", "pubmed"):
        ...
        parts.append(
            f'<text x="{lx + 10}" y="{ly + 2}" fill="var(--color-text-secondary, #475569)" font-family="system-ui,sans-serif" font-size="8">{SOURCE_LABELS.get(key, key)}</text>'
        )
        lx += 50
    parts.append("</svg>")
    return "\n".join(parts)
...
def generate_sparkline_svg(
    data: list[float], pillar: str = "aml", width: int = 60, height: int = 24
) -> str:
