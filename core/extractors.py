...
_DATE_PATTERNS = [
    ...
    re.compile(r"(?<![\w.])(?:in|by|since|until|from|of|after|before|around|between|during|c\.)\s+(\d{4})\b", re.IGNORECASE),
...
_STEP_PATTERNS = [
    ...
    re.compile(r"(\w[\w\s]+?)\s*[\u2192\u27a1\u27a4\u25b8>]\s+(\w[\w\s]+?)(?=[,.;]|(?:\s+[\u2192\u27a1\u27a4\u25b8>])|$)"),
            ...
            if pat_idx == 3:
                ...
                for pair in [(src, dst)]:
                    ...
                    if description not in seen and len(description) > 10:
                        candidates.append({"step": len(candidates) + 1, "description": description[:120]})
                ...
                for part in g:
                    if part:
                        ...
                        if desc not in seen and len(desc) > 10:
                            candidates.append({"step": len(candidates) + 1, "description": desc[:120]})
                            seen.add(desc)
            else:
...
_ANALYSIS_ENTITIES_RE = re.compile(r"\*\*Key entities:\*\*\s*(.*?)(?:\*\*Key numbers|\*\*SQI|\*\*From articles|$)", re.DOTALL)
_ANALYSIS_NUMBERS_RE = re.compile(r"\*\*Key numbers:\*\*\s*(.*?)(?:\*\*SQI|\*\*From articles|\*\*Key entities|$)", re.DOTALL)
