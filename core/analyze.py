...
from .score import build_history, compute_signal_score
                ...
                if ent.lower() in title_lower:
                    for pname in PILLARS:
                        if ent.lower() in (e.lower() for e in KNOWN_ENTITIES.get(pname, set())):
                            scores = [
                                (pname, scores[0][1] * 1.5) if s[0] == pname else (s[0], s[1] * 0.8)
                                for s in scores
                            ]
                            break
                    break
...
def classify_bloom_level_enhanced(story: dict, scraped_text: str | None = None) -> str:
    ...
    return max(level_scores, key=lambda k: level_scores[k])
...
def detect_trending_topics(stories: list[dict]) -> list[dict]:
    ...
    trending = []
    total_days = max(1, len(set(d for t in trends.values() for d in t.get("dates", []))))
...
def build_pillar_signals(
    stories: list[dict], pillar_name: str, scraped: dict[str, dict] | None = None
) -> dict:
    ...
    for s in stories:
        ...
        if scraped:
            ...
            for sent in facts.get("sentences", []):
                ...
                nums = re.findall(
                    r"\$?(\d[\d,]*\.?\d*)\s*(million|billion|trillion|mln|bln|%)?", sent, re.I
                )
    ...
    top_sqi = sorted(
        [(s, q) for s, q in zip(stories, sqi_scores)], key=lambda x: x[1]["sqi"], reverse=True
    )[:3]
    ...
    return {
        ...
        "entity_coherence": "high"
        if entity_counts and len(entity_counts) < max(2, len(stories) * 0.4)
        else "low",
...
def generate_popular_summary(
    stories: list[dict], signals: dict, pillar_name: str, config: dict
) -> str:
    ...
    if signals.get("domain_diversity", 0) >= 3:
        ...
        midsection += (
            f" Discussion centers mainly on a source from the **{signals['top_domain']}** category."
        )
    ...
    if trending:
        midsection += (
            " In today's articles, trending topics include: "
    ...
    if key_nums:
        midsection += " Key numbers include: " + "; ".join(f"{n[0]}" for n in key_nums[:3]) + "."

    entities = signals.get("top_entities", [])
    if entities:
        midsection += " Key entities: " + ", ".join(f"**{e}**" for e in entities[:4]) + "."
...
def build_analysis(
    ...
    scraped: dict[str, dict] | None = None,
) -> dict:
