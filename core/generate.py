...
from .analyze import (
    build_analysis,
    classify_bloom_level_enhanced,
    compute_cross_pillar_scores,
)
from .bloom import (
    generate_flashcards,
...
from .data import BASE_DIR, PILLARS, log
...
from .scraper import _url_key, scrape_articles
from .visuals import (
    generate_og_image,
...
def _build_content_deep_analysis(
    pillar_stories: list[dict], scraped: dict[str, dict], pillar_name: str, signals: dict
) -> str:
...
def _build_cross_pillar_section(pillar_name: str, all_pillar_stories: dict[str, list[dict]]) -> str:
    ...
    {p: s for p, s in all_pillar_stories.items() if p != pillar_name}
    ...
    for s in this_pillar_stories:
        ...
        for p, score in cross.items():
            if score >= 4:
                ...
                connections.append(
                    f'- "{s["title"][:60]}" has connections to **{label}** (score: {score})'
                )
...
def _build_classification_confidence(
    stories: list[dict], pillar_name: str, pillar_stories: list[dict], unclassified: int
) -> str:
...
def generate_post(
    ...
    post_dir: Path = config["folder"] / bundle_name
    ...
    cross = _build_cross_pillar_section(pillar_name, all_pillar_stories or {})
    if cross:
        lines.extend([cross, ""])
        ...
        if img_url:
            ...
            if img_url.startswith("/"):
                ...
                if article_url:
                    ...
                    log(
                        f"Skipping image download: no base URL for relative path {img_url}",
                        ok=False,
                    )
                    img_url = None
    ...
    if pillar_stories:
        ...
        quality_metrics["avg_source_score"] = (
    ...
    if thumb_filename:
        assets.append(
            build_asset_manifest(
                content_id,
                "thumbnail",
                post_dir / thumb_filename,
                top_story.get("url", "") if top_story else "",
            )
        )
    if og_filename:
        assets.append(
    ...
    if top_story:
        ...
        if img_url:
            ...
            if feat_path.exists():
                assets.append(
                    build_asset_manifest(content_id, "featured_image", feat_path, img_url)
                )
    ...
    log(
        f"Generated: {filepath.relative_to(BASE_DIR)} ({link_count} links, SQI={signals.get('avg_sqi', 0):.3f})"
    )
    return filepath
