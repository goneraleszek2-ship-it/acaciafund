...
def build_asset_manifest(content_id: str, asset_type: str, path: Path, source_url: str = "") -> dict[str, Any]:
...
def build_registry_index() -> dict[str, Any]:
    story_root = BASE_DIR / "content" / "pl" / "blog"
    run_root = RUNS_DIR
    ...
    for path in _manifest_files(story_root, "*/manifest.json"):
        manifest = _safe_read_manifest(path)
        if not manifest or manifest.get("manifest_type") != "story":
            continue
        page = _story_manifest_summary(path, manifest)
        pages.append(page)
        content_id = page["content_id"]
        if content_id:
            page_by_id[content_id] = page
        pillar = page["pillar"] or "unknown"
...
def get_story_manifest(content_id: str) -> dict[str, Any] | None:
    ...
    return index.get("by_content_id", {}).get(content_id)


def get_latest_story_manifest(pillar: str) -> dict[str, Any] | None:
    index = load_registry_index()
    content_id = index.get("latest_by_pillar", {}).get(pillar)
    if not content_id:
        return None
    return index.get("by_content_id", {}).get(content_id)
...
def get_run_manifest(run_id: str) -> dict[str, Any] | None:
    index = load_registry_index()
    return index.get("by_run_id", {}).get(run_id)
