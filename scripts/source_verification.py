...
def classify_source_type(url: str = "", tags: list = [], content_type: str = "", title: str = "") -> tuple[str, float]:
    """Classify source type and trust score."""
    url_lower = url.lower() if url else ""
    
    # Academic sources
    if any(x in url_lower for x in ["arxiv.org", "pubmed.ncbi.nlm.nih.gov", "sciencedirect.com", "springer.com", "tandfonline.com", "ieee.org", "acm.org"]):
        return "academic", 0.95
    
    # Official sources
    if any(x in url_lower for x in [".gov", ".edu", ".org", "apache.org", "kubernetes.io", "docker.com", "python.org", "rust-lang.org", "developer.mozilla.org", "docs.microsoft.com", "developer.apple.com", "google.com", "aws.amazon.com", "azure.microsoft.com"]):
        return "official", 0.9
    
    # Industry reports
    if any(x in url_lower for x in ["mckinsey.com", "bcg.com", "gartner.com", "idc.com", "forrester.com", "statista.com", "marketwatch.com", "bloomberg.com", "reuters.com"]):
        return "industry_report", 0.85
    
    # Engineering blogs
    if any(x in url_lower for x in ["aws.amazon.com/blogs", "cloud.google.com/blog", "dev.to", "medium.com", "hashnode.com", "towardsdatascience.com", "towardsai.net"]):
        return "engineering_blog", 0.75
    
    # News sources
    if any(x in url_lower for x in ["nytimes.com", "washingtonpost.com", "bbc.com", "cnn.com", "foxnews.com", "nbcnews.com", "cbsnews.com"]):
        return "news", 0.7
    
    # Social media
    if any(x in url_lower for x in ["twitter.com", "x.com", "linkedin.com", "youtube.com", "tiktok.com", "instagram.com"]):
        return "social", 0.5
...
def main():
    ...
    stats = {
        ...
        "verified_count": int((df['verified'] == True).sum()),
        "unverified_count": int((df['verified'] == False).sum()),
