...
def create_signals_table(conn: duckdb.DuckDBPyConnection, articles: list[dict]) -> None:
    ...
    if len(df) > 0:
        ...
        conn.execute(
            "CREATE TABLE signals (slug TEXT, avg_sqi FLOAT, avg_score FLOAT, count INTEGER, domain_diversity INTEGER, total_score INTEGER)"
        )
...
def main():
    ...
    print("\nView Statistics:")
    print(
        f"  Articles with SQI: {conn.execute('SELECT COUNT(*) FROM articles_with_sqi').fetchone()[0]}"
    )
    print(
        f"  High quality articles: {conn.execute('SELECT COUNT(*) FROM high_quality_articles').fetchone()[0]}"
    )
