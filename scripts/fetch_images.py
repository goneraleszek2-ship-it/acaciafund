...
def build_fallback_queries(section_type: str, pillar: str) -> list[str]:
    ...
    pillar_kw = (
        PILLAR_KEYWORDS.get(pillar, "technology data").split()[0] if pillar else "technology"
    )
...
def compute_break_points(sections: list[dict], article: dict) -> list[dict]:
    ...
    always = [
        s
        for s in sections
        if SECTION_PRIORITY.get(s["section_index"]) == "always" and s["word_count"] > 0
    ]
    conditional = [
        s
        for s in sections
        if SECTION_PRIORITY.get(s["section_index"]) == "conditional"
...
def build_section_query(section: dict, article: dict) -> str:
    ...
    title_core = re.sub(r"[:;]+", " ", title_core).strip()[:80]
    ...
    # Filter noise: remove terms unlikely to yield visual results
    # (high-signal tech/domain terms are intentionally allowed through)
    noise_terms = {
        "acaciafund",
        "jwst",
        "defi",
...
def _match_curated_keywords(keywords: list[str], text: str) -> bool:
    """Relaxed curated-keyword matching.

    For short keyword lists (<3 terms), any single match suffices.
    For longer lists, at least 2 keywords must appear in the text.
    This increases recall over the previous all-or-nothing approach.
    """
    if len(keywords) < 3:
...
def resolve_curated(article: dict) -> str | None:
    ...
    for phrase, filename in CURATED_KNOWN.items():
        ...
        if _match_curated_keywords(keywords, haystack) or _match_curated_keywords(
            keywords, body_text
        ):
            return filename
    return None
...
def search_loc(query: str) -> list[dict]:
    ...
    try:
        resp = requests.get(
            "https://www.loc.gov/pictures/search/",
            params={
                "q": query,
                "fo": "json",
                "at": "pict",
            ...
            candidates.append(
                {
                    ...
                    "creator": r.get("contributor", [{}])[0].get("name", "")
...
def generate_svg_placeholder(prompt: str, dest: Path) -> tuple[bool, str, int, int, int]:
    ...
    for i in range(n // 2):
        ...
        shapes.append(
            f'<rect x="{x1}%" y="{y1}%" width="{w}%" height="{hr}%" rx="4" fill="none" stroke="{accent2}" stroke-width="0.5" opacity="0.3"/>'
        )
...
def download_image(url: str, dest: Path, retries: int = 0) -> tuple[bool, str, int, int, int]:
    for attempt in range(retries + 1):
        try:
            resp = requests.get(
                url, headers={"User-Agent": USER_AGENT}, timeout=(3, 8), stream=True
            )
            resp.raise_for_status()
                ...
                manifest_results.append(
                    {
                        ...
                        "content_hash": hashlib.sha256(
                            section.get("text_content", "").encode()
                        ).hexdigest()[:16],
                    }
                )
        if manifest_results:
    ...
    for section in break_sections:
        ...
        if not force and idx in existing_indices:
            existing_entry = next(
                (
                    s
                    for s in (article.get("section_images", []) or [])
                    if s.get("section_index") == idx
                ),
                None,
            )
            if existing_entry:
                ...
                if img_path and (PROJECT_ROOT / img_path.lstrip("/")).exists():
                    ...
                    used_creators.add(
                        existing_entry.get("image_credit", "").split("via")[0].strip().lower()
                    )
                    _GLOBAL_USED_URLS.add(img_path)
                    continue
        ...
        ] = []  # (candidate, backend, query_used, width, height)
                ...
                for attempt in range(1):
                    try:
                        ...
                        for c in candidates:
                            ...
                            if (
                                creator_key
                                and _GLOBAL_USED_CREATORS.get(creator_key, 0)
                                >= MAX_IMAGES_PER_CREATOR
                            ):
                                continue
                            # Pre-filter negative keywords (Phase 4)
                            tags_lower = (
                                c.get("tags", "").lower() + " " + c.get("title", "").lower()
                            )
                            if any(
                                n in tags_lower for n in {"screenshot", "logo", "icon", "cartoon"}
                            ):
                                continue
...
def _build_featured_queries(article: dict) -> list[str]:
    ...
    broad_kw = {
        "aml": "financial regulation banking compliance",
        "stock": "market trading semiconductor investment",
        "data-engineering": "data pipeline database technology",
        "science": "research laboratory experiment science",
    }.get(pillar, "technology")
...
def save_pipeline_stats(stats: dict) -> None:
    ...
    record = {
        ...
        "section_coverage": {
            k: {"filled": v[0], "total": v[1]} for k, v in stats.get("section_coverage", {}).items()
        },
        "relevance_scores": stats.get("relevance_scores", []),
        "avg_score": round(
            sum(stats.get("relevance_scores", [])) / max(len(stats.get("relevance_scores", [])), 1),
            1,
        ),
        "total_bytes": stats.get("total_bytes", 0),
        "fill_rate": round(
            stats.get("filled_slots", 0) / max(stats.get("total_section_slots", 1), 1) * 100, 1
        ),
...
def print_report(stats: dict):
    ...
    if scores:
        ...
        print(
            f"  Above threshold (70+):  {sum(1 for s in scores if s >= 70)} ({sum(1 for s in scores if s >= 70) / max(len(scores), 1) * 100:.0f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--re-evaluate",
        action="store_true",
        help="Re-score cached section images below 70 threshold and re-fetch if better",
    )
    parser.add_argument(
        "--cache", action="store_true", help="Skip articles that already have images"
    )
    ...
    for article in articles_to_process:
        ...
        if use_cache and existing_images and not args.force:
            ...
            if valid_images:
                ...
                if fully_covered and not (
                    args.re_evaluate
                    and any(img.get("relevance_score", 0) < 70 for img in valid_images)
                ):
                    ...
                    if not feat or not (PROJECT_ROOT / feat.lstrip("/")).exists():
                        ...
                        if fi:
                            if validate_image_for_article(article, fi):
                                article["featured_image"] = fi
                                updated_count += 1
                            else:
                                fpath = PROJECT_ROOT / fi.lstrip("/")
                                if fpath.exists():
                                    fpath.unlink()
                                print(f"  ✗ rejected featured image (no semantic overlap): {fi}")
        ...
        # Fetch featured image if missing — with semantic validation
        feat = article.get("featured_image", "")
        if not feat or not (PROJECT_ROOT / feat.lstrip("/")).exists():
            fi = fetch_featured_image(article)
            if fi:
