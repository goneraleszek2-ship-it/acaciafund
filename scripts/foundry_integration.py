...
from foundry_dev_tools import Config, FoundryContext, JWTTokenProvider  # noqa: E402
...
def fetch_dataset_schema(ctx, dataset_path: str) -> dict | None:
    ...
    try:
        ...
        schema = ctx.foundry_rest_client.get_dataset_schema(
...
def download_dataset_data(ctx, dataset_path: str, output_dir: str) -> list[str]:
    ...
    try:
        ...
        files = ctx.foundry_rest_client.download_dataset_files(
            dataset_rid=dataset_rid, output_directory=output_dir, view="master"
        )
        return files
    except Exception as e:
    ...
    if command == "test":
        ...
        if schema:
            ...
            try:
                ...
                fields = schema.get(
                    "fieldSchemaList", schema.get("schema", {}).get("fieldSchemaList", [])
                )
                if fields:
                    for field in fields:
