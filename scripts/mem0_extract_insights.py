...
from services.mem0 import save_insight, extract_from_commit
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract insights from Git commit messages"
    )
    parser.add_argument(
        "message",
        nargs="?",
    ...
    parser.add_argument(
        "--commit",
        help="Git commit hash to extract message from"
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Output parsed insight as JSON"
    )
    ...
    if args.json:
        ...
        print(f"Extracted insight from commit:")
        print(f"  Type: {insight['type']}")
        print(f"  Title: {insight['title']}")
        print(f"  Tags: {', '.join(insight['tags'])}")
        print(f"  Files: {', '.join(insight['related_files']) if insight['related_files'] else 'None'}")
