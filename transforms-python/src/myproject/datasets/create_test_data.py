...
@lightweight(test_source=LightweightOutput(DatasetPaths.SOURCE_DATASET) # <-- Change here)
    ...
    df = pd.DataFrame({
        ...
        "source_api": ["arxiv" if i % 5 == 0 else "pubmed" if i % 5 == 1 else "curated" for i in range(1, 101)],
        "domain": ["acaciafund.org"] * 100,
        "tags": [[f"tag_{j}" for j in range(1, (i % 5) + 2)] for i in range(1, 101)],
        "pillar": ["AML" if i % 3 == 0 else "Markets" if i % 3 == 1 else "Data Engineering" for i in range(1, 101)],
        ...
        "adoption_level": ["emerging" if i % 3 == 0 else "experimental" if i % 3 == 1 else "mainstream" for i in range(1, 101)],
