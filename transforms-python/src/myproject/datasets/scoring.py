...
def compute(
    ...
    try:
        ...
        df = df.with_columns([
            ...
            pl.lit("v2.0").alias("scoring_version"),
        ])
        ...
        print(f"Scoring completed: {df.select(pl.len()).collect()} records")
...
@transform.using(
    ...
    sources=Input(DatasetPaths.SOURCE_METADATA),
)
def source_verification(
    ...
    try:
        ...
        df = df.with_columns([
            ...
            pl.lit("v2.0").alias("verification_version"),
        ])
        ...
        print(f"Error in source_verification: {str(e)}")
        raise
